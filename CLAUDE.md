...
- **`auth.js`** — localStorage-only mock auth (`Auth`). Per-user storage keys come from `Auth.key(base)` → `sj_u_<id>_<base>`; all user data (stats, history, custom categories, saved game) goes through `uRead`/`uWrite` in game.js using these keys. README documents the intent to swap this for Firebase Auth while keeping the same surface (`Auth.current()`, `Auth.key()`, `Auth.onChange()`).
